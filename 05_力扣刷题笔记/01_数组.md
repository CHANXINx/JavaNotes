...
- **标签：** #数组 #哈希表 
### 思路一：倒序遍历
可以通过倒序遍历来判断最少需要操作几次，因为只有倒序遍历能首先获得最后的重复元素。

具体而言，若出现重复元素，则计算`i/3`，并向上取整，也就是`(i/3)+1`.
```java
class Solution {
...
- **标签：** #数组 #哈希表 
### 思路一：
仔细思考，其实就是计算大于k的不同元素的个数。并且若出现小于k的数，说明此时不可能全部变为k，返回-1即可。
```java
class Solution {
...
\end{align}
$$
因为`lower<=ai<=upper`，因此有`lower - si <= a0 <= upper -si`，因此交集数量为`upper-max{si} - lower-min{si} + 1`！故我们遍历difference数组，计算`min{si}`和`max{si}`即可。
```java
class Solution {
...
由于右移窗口时，可能会导致窗口内包含某个元素多次，因此需要使用哈希表来存储窗口内某个元素的个数。当需要缩小窗口时，就不断左移，并扣减对应元素的出现次数，直至窗口内不同元素的数量不符合条件。
```java
class Solution {
...
- **标签：** #前缀和 #数组 #哈希表 
### 思路一：前缀和
统计子数组中满足`nums[i] % modulo == k`的元素的个数`cnt`，当cnt符合`cnt % modulo == k`时，说明子数组符合条件。

子数组问题，很容易联想到滑动窗口或者前缀和来解决。

我们利用前缀和统计前i个元素中，符合条件的元素个数。获取前缀和数组后，我们就可以通过遍历前缀和数组、利用哈希表存储元素出现个数来获取结果。

数学推导：
`cnt[r] - cnt[l] % modulo == k`，此时我们需要找到符合条件的`cnt[l]`.
由于`k < modulo`，因此`k == k % modulo`，即有`cnt[r] - cnt[l] % modulo == k % modulo`！即`cnt[r] - cnt[l]` 与`k`同余。
由于模运算加减法封闭，则有`(cnt[r] - k) % modulo == cnt[l] % modulo`，故我们可以在遍历过程中存储`cnt[i] % k`的出现次数，结果即为获取哈希表中的`cnt[r] - k`的计数。
```java
class Solution {
    public long countInterestingSubarrays(List<Integer> nums, int modulo, int k) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            cnt[i] = (nums.get(i-1) % modulo == k ? 1 : 0) + cnt[i-1];
        }
        // [0,1,1,1];
        long ans = 0;
        // freq: 记录每个前缀 cnt[i] % modulo 出现的次数
        Map<Integer, Long> freq = new HashMap<>();
        // 空前缀（i=0）余数为 0，出现一次
        freq.put(0, 1L);

        for (int i = 1; i <= n; i++) {
            ans += freq.getOrDefault((cnt[i]-k) % modulo,0L);
            freq.put(cnt[i] % modulo, freq.getOrDefault(cnt[i] % modulo, 0L) + 1);
        }
...
- **标签：** #数组 #双指针 #滑动窗口 
### 思路一：双指针
需要获取定界子数组，我们可以一次遍历，获取`minK`和`maxK`出现的最近位置。遍历过程中枚举右端点`i`，获取以当前右端点`i`作为结尾的符合条件的子数组。

为了保证子数组中不含有不符合条件的元素，因此需要额外记录`<minK || > maxK`的元素`i0`的出现位置。因此，以`i`作为右端点的子数组，符合条件的有`(Math.min(minK, maxK) - i0, 0)`个。

`Math.max(Math.min(minK, maxK) - i0, 0)`相当于排除`i0`在子数组中的情况。因为`-i0`小于0说明`min(minK, maxK)`小于`i0`。
```java
class Solution {
...
- **标签：** #数组 #滑动窗口 
### 思路一：
子数组问题，自然联想到滑动窗口方法。并且由于数组全是正数，而我们要统计得分小于K的子数组，因此是「越短越合法」型滑动窗口。并且当子数组满足条件，其内部的子数组也都满足条件。

对于此类问题，我们移动右窗口，直至找到不符合条件的子数组，然后再移动左窗口，
```java
class Solution {
