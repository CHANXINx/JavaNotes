...
- **标签：** #动态规划 
### 思路一：
- **dp数组定义**：`dp[i][j]`表示word1中前i个字符`substring(0,i)`变换到word2中前j个字符`substring(0,j)`，需要操作的次数。
	- 这里注意，定义成`dp[m+1][n+1]`，是为了保证空字符串也能正确处理。例如m=0，若定义成`dp[m=0][n]`，后续无法进行求解了。
...
class Solution {
    public int minimumWhiteTiles(String floor, int numCarpets, int carpetLen) {
        ...
        // dp[i][j]表示用i条地毯覆盖下标[0,j]的地砖，所能获得的最少白色砖块数目
        int[][] dp = new int[numCarpets+1][n];
        // 初始化：未盖地砖，则dp[0][j] = sum(s[j]-'0');
        for (int j = 0; j < n; j++){
            dp[0][j] = (s[j] - '0') + (j-1<0 ? 0 : dp[0][j-1]); 
        }
        // 递推公式：盖则dp[i-1][j-capLen],不盖则dp[i][j]+当前地砖.
        for (int i = 1; i <= numCarpets; i++){
            for (int j = carpetLen * i; j < n; j++) {
                dp[i][j] = Math.min(dp[i-1][j-carpetLen], dp[i][j-1] + s[j]-'0');
            }
        }
...
```
**时间复杂度：** $O(numCarpets * n)$
**空间复杂度：** $O(numCarpets * n)$
## 132. 分割回文串 II
>**时间**：2025-03-02 10:28:18
### 题目
...
- **标签：** #动态规划 #字符串 
### 思路一：
1. 预处理`g[l][r]`，避免后续都需要使用双指针扫描来判断是否回文。
	- 预处理时，r指针从小到大，l指针从大到小，因为`g[l][r]`是否回文需要依赖于`g[l+1][r-1]`.
2. **dp数组定义**：`dp[i]`表示`[1,i]`所需的最小分割次数。
3. **dp数组初始化**： 初始化为一个不可达的数即可，以便后续的更新。
4. **递推公式**：
	- 若只有一个字符，则回文。
	- 若两端字符相等，即`s[l-1] == s[r-1]`，且只有两个字符或`g[l+1][r-1]`已回文，则当前也回文。
5. **遍历顺序**：`i`遍历`[1,n]`，`j`遍历`[j,i]`.
```java
class Solution {
    public int minCut(String S) {
        // 预处理g[l][r]: g[l][r]表示[l,r]这一段是否为回文串.
        int n = S.length();
        char[] s = S.toCharArray();
        ...
        for (int i = 1; i <= n; i++) {
            ...
            if (g[1][i]) {
                ...
                for (int j = 1; j <= i; j++) {
                    if (g[j][i]) {
                        // dp[j-1]代表[1,j-1]分割的次数, +1代表[j,i]回文, 增加多一次分割次数.
...
- **标签：** #动态规划 #字符串 
### 思路一：
在计算`minChange`时，i是从索引n-2开始的，因为若从n-1开始，则j从i+1=n开始，数组会越界。并且`minChange[n-1][n]`实质上是无意义的。毕竟`minChange[i][j]`表示的是子串`[i,j]`变成回文的最小分割次数，要求j是大于i的！

- **dp数组定义**：`dp[i][j]`表示将子串`[0,j]`划分为`i+1`个非空回文子串，所需的最少修改次数。
- **dp数组初始化**：`dp[0] = minChange[0]`表示不进行划分时，要使得`dp[0][r]`回文，即需要修改次数`minChange[0][r]`.
- **递推公式**：`dp[i][r]`所需的最少修改次数，为`dp[i-1][j-1] + minChange[j][r]`，其中`j = [i,r]`。意思是将`[0,r]`划分为`i+1`个子串，为`dp[i-1][j-1] + minChange[j][r]`的最小值。`dp[i-1][j-1]`表示`[0,j-1]`已划分为i个非空子串，`minChange[j][r]`表示将`j,r`也变为非空子串所需的修改次数。二者相加即为划分为i+1个非空子串所需的修改次数。
```java
class Solution {
    public int palindromePartition(String S, int k) {
        // 定义minChange[i][j]表示将子串[i,j]修改为回文子串所需的最少修改次数.
        // dp[i][j]表示子串[0,j]划分为i+1个非空子串,且每个子串均为回文,所需的最少次数.
        // 最终答案: dp[k-1][j-1].
        // 递推公式: minChange[i][j] = minChange[i+1][j-1] + s[i]==[j] ? 1 : 0.
        //          dp[i][j] =  todo.
        ...
        for (int i = 1; i < k; i++) {
            ...
            for (int r = i; r < n; r++) {
				...
				// dp[i][r] = min(dp[i-1][l-1]) + minChange[l][r];
                for (int l = i; l <= r; l++) {
                    dp[i][r] = Math.min(dp[i][r], dp[i-1][l-1] + minChange[l][r]);
                }
            }
        }
        return dp[k-1][n-1];
...
- **标签：** #动态规划 #记忆化搜索 #回溯 
### 思路一：
计算方案数，相当于爬楼梯问题，其中zero和one的值就是一次能爬的楼梯的个数！！！由于好字符串的长度在low和high之间都行，因此最终结果将`dp[low]~dp[high]`累加即可。
```java
class Solution {
    private static final int MOD = 1_000_000_007;
