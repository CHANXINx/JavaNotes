...
##### 单机性能问题
由以下2个方式来解决：
1. 利用阻塞队列BlockingQueue实现生产者-消费者模式，生产者只负责扫描需要关闭的订单，消费者只负责执行关闭订单操作。
2. 生产者和消费者都采用多线程ForkJoinPool组成，以提高并发性能！
...
## 15. MOCK支付模块时基于TTL的支付回调参数传递


## 16. 基于Seata的支付环节的数据一致性
...
## 17. 基于Seata的事务钩子Hook+定时任务保证上链的一致性
