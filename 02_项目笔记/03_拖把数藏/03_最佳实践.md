...
## 5. 自定义 TypeHandler 实现隐私数据自动加解密【用于用户实名认证】
#### 基本介绍 
...
- 在MyBatis的xml配置文件中配置TypeHandler。![[Pasted image 20250223022843.png]]
...
13. **作为基础模块，其余很多模块都依赖于用户模块，需要查询用户信息；**
14. 用户信息变更不频繁，属于**读多写少**的场景，因此适用于添加缓存！

综上，为了在高并发的场景下的快速响应、减少加载慢的情况，以及**保护用户服务**，为用户模块添加了二级缓存。

**不过，不添加也是合理的。**
## 12. 基于Redis+MQ+数据库实现高并发扣减
...
```
## 15. 基于ShardingSphere的订单表分表 【TODO】
...
## 16. 基于RocketMQ的订单取消
15. 订单模块发送半消息给RocketMQ；
16. 半消息发送成功，则进行订单取消操作；
17. 订单取消成功，则发送COMMIT消息，MQ再通知藏品模块进行库存回滚；订单取消失败，则发送RollBack回滚操作，取消操作。
	1. **若COMMIT或ROLLBACK消息（即第二个半消息）发送失败**，则需要MQ回调反查订单模块，判断订单是否取消成功来决定是否发送消息给藏品模块进行库存的回滚操作。


## 17. 基于XXL-JOB实现订单的超时关闭
...
## 18. MOCK支付模块时基于TTL的支付回调参数传递


## 19. 基于Seata的支付环节的数据一致性
...
## 20. 基于Seata的事务钩子Hook+定时任务保证上链的一致性
...
## 22. Convertor实现参数映射
21. 高频场景需求、复杂的映射关系。
22. 简单的转换直接调用Spring的BeanUtils即可。
基于MapStruct

## 23. 重构"库存扣减Confirm的并发问题"
