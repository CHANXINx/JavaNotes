...
- 若`hours[i]=30`,则此时与`i<j`且**余数为6**的`hours[j]`的数目相加即为正确的组合数
- 若`hours[i]=2`,则此时与`i<j`且**余数为2**的`hours[j]`的数目相加即为可能的组合数.
	- 推导出: `ans += cnt[24 - (hours[i] % 24)]`
- 若`hours[i]==24,48..`,则此时与`i<j`且余数为0的`hours[j]`的数目相加即为正确组合.
### 代码:
```java
class Solution {
