...
```
## 11. JDK21中的虚拟线程是什么？
新引入的虚拟线程，是JDK 实现的**轻量级线程**，他可以**避免线程上下文切换带来的的额外耗费**。他的实现原理其实是JDK**不再是每一个线程都一对一的对应一个操作系统的线程**，而是会**将多个虚拟线程映射到少量操作系统线程**中，通过有效的调度来避免那些上下文切换。
...
## 14. 为什么`wait`和`notify`方法必须在`synchronized`代码块中使用？

## 15. `wait`和`notifiy`的虚假唤醒的产生原因及如何解决？
#### 是什么及产生原因： TODO
...
## 31. 创建线程的开销是什么？
总览来说，开销主要分为两部分：时间开销、内存开销。
手动创建线程操作：
```
Thread t1 = new Thread();
t1.start();
```
调用`new Thread()`方法时，实质上只是初始化了线程，在调用`start()`方法时，才是真正调用本地方法创建线程！
...
- `handler`：拒绝策略。当核心线程都在忙、且工作队列也已经满了，那么此时新任务就会被拒绝。
	- 常见的拒绝策略：
		...
		5. 自定义：实现`RejectedExecutionHandler` 接口，实现`rejectedExecution`方法。
## 5. 线程池的拒绝策略有哪些？
...
**原始的CLH队列**：
- 基于自旋监听前驱节点，每个线程的节点不断检查前驱节点的状态（waitStatus），前驱释放锁后，当前线程才能获取锁；
- 公平性保证，严格按照入队顺序分配锁，避免饥饿。

**AQS的CLH队列变体**：
- 将自旋改为阻塞等待，避免CPU资源的浪费；锁释放后，通过`unpark`唤醒后驱线程来获取锁；
- 双端队列（双向链表），有`prev`和`next`指针，便于快速删除中间节点；
- 状态扩展，维护waitStatus变量，用户标识线程状态和唤醒需求。
