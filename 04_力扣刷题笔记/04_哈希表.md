...
**具体算法流程：**
7. 遍历$i$从$0\sim n-1$，目的是让当前i上的索引回到其应在的位置上；
	- 每遍历一个索引，都会保证有$nums[i]==nums[nums[i]-1]$，或者$nums[i]<=0 || nums[i]>n$（即非法）。
8. 再次遍历，判断位置不符的元素；若所以位置都相符，则返回$n+1$。
```java
class Solution {
...
### 思路一：Map哈希表
9. 利用Map作哈希表，保存元素数量；
10. 利用`values()`方法获取每个元素对应的数量，并调用`list.sort()`方法进行倒排；
11. 贪心获取元素数量，若ans已超过$\frac{n}{2}$，则直接返回。
```java
class Solution {
...
**具体算法流程：**
12. 定义二维数组`cnt[26][teamNum+1]`，第一维表示队伍，第二维表示票数；
13. 将cnt数组中的`cnt[teamNum]`设为对应字母，便于后续票数相同时根据字母排序，以及最后的结果构造；
14. 遍历votes数组，更新每个队伍的票数；
15. 数组排序，自定义排序规则：先按照票数排序，最后才按照字母排序。由于票数有多个，因此需要使用for循环，相当于选票1相同，按照选票2排序，选票2相同，按照选票3排序等等等；
16. 构造答案。
```java
class Solution {
