...
`Change buffer`，变更缓冲区，用于缓存当二级索引页不在`buffer pool`时的变更操作（DML操作）。这些变更操作会在之后当其他读取操作将索引页读取到`buffer pool`时应用。
##### 为什么需要change buffer？
若每次DML操作都将
...
#### 1. Master Thread
#### 2. IO Thread
执行异步IO操作（AIO模型），例如主从时，IO Thread会负责与主库进行连接。
#### 3. Purge Thread
在事务提交后，可能对应的undo log不再被需要，此时可以清除undo log。这个清除操作就是通过后台线程Purge Thread进行的。
...
![[Pasted image 20250425103846.png|1200]]
### row:
binlog中会**记录每个数据更改的具体行的细节**，每条日志都会详细记录发生变更的行的内容和修改情况。

缺点是需要记录更多的内容，如批量修改，需要将每条记录的变更细节都记录。因此**数据恢复时需要更长时间**。

![[Pasted image 20250425103854.png|800]]
需要通过mysqlbinlog工具读取row形式的日志。
### mixed:
两者的结合，**自动切换选择**合适的格式进行记录。但是在RC级别下，只有row格式可以生效。

MySQL 自己会判断这条 SQL 语句**是否可能引起主备不一致**，如果有可能，就用 row 格式，否则就用 statement 格式。【也就是说，默认使用statement】
## 为什么使用statement形式进行主从同步可能会导致数据不一致？
因为使用命令更新恢复， 是需要上下文信息的。若主库和从库执行同一个语句，使用了不同的索引，那么就有可能导致不一致。
...
两阶段提交**确保了binlog和redolog的数据一致性**，从而保证数据库在崩溃恢复等场景下的可靠性和稳定性。
...
## MySQL的主从同步过程是怎样的？
- 主库会根据从库传过来的位置，**从本地读取binlog发给从库**；
- 从库接收到binlog后，会写到本地文件，称为中转日志relay log。
- 从库的`sql thread`会**读取、解析中转日志中的命令并执行**。

注意，这里使用的binlog日志应该是statement形式的，
## 4. 分库分表会带来什么问题？
1. 跨库事务问题；
2. 跨多表的分页、排序；
3. 二次分表的数据迁移问题；
