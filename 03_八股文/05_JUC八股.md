...
`InheritableThreadLocal`用于**父子线程之间的参数传递**，但是只适用于在主线程中手动创建子线程。而无法在复用线程的线程池中使用。
...
### 锁粗化：
- JIT发现一系列连续的操作都对同一个对象反复加锁和解锁，甚至加锁操作出现在循环体中的时候，会**将加锁同步的范围扩散（粗化）到整个操作序列的外部**。
```java
