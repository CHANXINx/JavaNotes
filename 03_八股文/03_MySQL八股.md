...
7. 聚簇索引也叫主键索引，非聚簇索引也叫二级索引。
8. **数据存储**：聚簇索引的叶子结点包含了索引主键和对应的整行数据；而非聚簇索引的叶子结点不包含完整数据，只包含对应指向该行数据的主键值。
9. **索引和数据的关系**：进行查询时，通过聚簇索引可以直接获取完整数据；而通过非聚簇索引查询，若查询了索引字段之外的字段，还需要回表查询。
10. **唯一性**：聚簇索引通常是基于主键构建的，因此每一个表只能有一个聚簇索引；而非聚簇索引，一个表可以有多个。
...
11. 主键字段具有唯一性，且不能为空。
12. 最好具有递增趋势，这样能减少插入删除带来的页分裂和内存碎片。
13. 不建议使用业务数据作为主键，因为无法预测业务字段未来是否会重用；
14. 通常采用自增id作为主键，但仅限于单机系统。对于分布式系统，需要采用分布式id方案。

## 7. 采用UUID和自增id作为主键，哪个好?/自增id作为主键的优点.
> [!abstract] 摘要
> 1. 占用内存；
> 2. 安全性；
> 3. 查询效率；
...
**优点：**
15. **全局唯一**：采用不同算法生成，几乎可以保证全球范围内唯一，避免了多台机器之间的主键冲突问题，因此也适用于分布式系统。
16. **不可预测性**：随机生成，很难被猜测出来，安全性较高。

**缺点：**
17. **占用空间大**：通常以字符串形式存储，占用空间比较大；
18. **不适合范围查询**：UUID不是自增的，所以不支持范围查询。范围查询通常是依赖于主键的顺序性来快速定位数据，如果主键非自增，通常需要扫描更多的数据来确认数据是否在指定范围内。
19. **效率低**：
...
### 自增ID:
**优点：**
20. **占用空间小**：自增ID是数字，占用内存远小于UUID，节省存储空间；
21. **顺序插入，减少页分裂**：ID是递增的，新记录一般插入在索引页的末尾，避免了随机写入带来的页分裂和性能开销；
22. **查询效率高**：ID自增，索引中的数据存储顺序和值一致。
	- 因此。进行范围查询时可以更快定位范围的起始点，提高查询效率；
	- 在物理存储上记录也较为连续，减少IO
23. **分页方便**：id连续自增，通过 `id > ? LIMIT ?` 的方式分页可以解决深度分页问题；
24. **索引效率**：保证索引顺序和插入顺序相同，减少了索引重排，提高了插入速度。

**缺点：**
25. **分库分表**：分库分表时，没办法依赖于单表的自增主键来做主键id，因为可能会发生重复；
26. **可预测**：ID是顺序自增的，所以是可预测的；
	- 例如竞对利用自增id获取订单量。
27. **可能用尽**：自增id的数据类型是int(4字节)或者bigint(8字节)，可能会存在用完的情况。

## 8. MySQL主键一定是自增的吗?
不一定。只是一般采用自增id作为主键，这样有几个好处：
28. 自增id是顺序插入的，可以**减少插入删除时的页分裂、页合并等问题**，提高写入性能；
29. 由于是顺序插入的，所以物理存储上也是连续的，因此范围查询时效率更高。
...
30. 模糊查询，如`like %xx`或者`like %xx%`；
31. 使用函数；
32. 表达式计算；
33. 字符串类型未带''，导致发生了类型转换；
34. 不遵循最左匹配原则，导致联合索引失效；
35. **WHERE语句中，如果OR前是索引列，而OR后的条件不是索引列，那么条件会失效。**
...
## 13. 什么是索引下推(Index Condition Pushdown)?
> [!abstract] 摘要
...
## 14. 什么是索引跳跃扫描(Index Skip Scan)?  TODO
...
36. 原子性：通过undo log来保证，事务失败时可以回滚。
37. 一致性：其他三个满足了，自然也就满足。
38. 隔离性：通过MVCC+版本号来实现；
39. 持久性：通过redo log和bin log来保证。
...
>2. Read View（不同时期创建）；
>3. 锁
40. **读未提交**：可以读取到其他事务还未提交的变更。
	- 读取最新版本的数据。
41. **读已提交**：可以读取到其他事务已提交的变更。
	- 通过创建Read View实现，在每个语句执行前都会生成一个Read View。
42. **可重复读**：事务内前后查询的数据保持一致，与事务启动时看到的数据一致。
	- 通过创建Read View实现，在第一个读请求时创建Read View。
43. **串行化**：事务是串行进行，当某个数据被多个事务读写时，后面请求的事务会处于阻塞等待状态。
	- 通过对数据加读写锁实现。
...
**举例：**
44. RR级别下，事务A开启事务，执行查询语句开启快照读；此时事务B新插入记录并提交；事务A再对新插入的数据进行更新，此时更新成功，说明发生了幻读。
45. RR级别下，事务A开启事务，执行查询语句开启快照读；此时事务B新插入记录并提交；事务A再对新插入的数据进行更新，此时更新成功，说明发生了幻读。
...
为了提高并发。
46. RC在加锁时，无需添加Gap Lock和Next Key Lock，只会对被修改的记录添加Record Lock，这使得RC级别的并发度会高于RR级别，同时减少死锁的发生；
47. RC支持“半一致读”，可以大大减少更新语句时行锁的冲突；对于不满足更新记录的条件，可以提前释放锁，提高并发度。
	- **半一致读**：RC支持“半一致读”，即在读取数据时，如果该数据被其他事务修改但尚未提交，MySQL会返回该记录最近提交的版本。
48. RC支持三种binlog格式，而RR支持持
## 10. 什么是半一致读？^
...
49. **行锁判断**：当一个更新语句的 `WHERE` 条件匹配到的记录已被其他事务锁定时，MySQL会首先返回最新已提交版本的记录。
50. **条件判断**：在确认读取到的数据后，再决定是否对该行加锁。如果当前事务持有的锁与要加锁的行不冲突，则可以提前释放锁，从而避免不必要的阻塞[2][4][6].
